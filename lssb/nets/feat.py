...
class FEAT(nn.Module):
    ...
    def forward(self, instance_embs, support_idx, query_idx):
        ...
        # for regularization
        if self.training:
            aux_task = torch.cat([support.view(1, self.shot, self.way, emb_dim), 
                                  query.view(1, self.query, self.way, emb_dim)], 1) # T x (K+Kq) x N x d
            num_query = np.prod(aux_task.shape[1:3])
            aux_task = aux_task.permute([0, 2, 1, 3])
            aux_task = aux_task.contiguous().view(-1, self.shot + self.query, emb_dim)
            # apply the transformation over the Aug Task
            aux_emb = self.slf_attn(aux_task, aux_task, aux_task) # T x N x (K+Kq) x d
            # compute class mean
            aux_emb = aux_emb.view(num_batch, self.way, self.shot + self.query, emb_dim)
            ...
            if self.use_euclidean:
                ...
                logits_reg = - torch.sum((aux_center - aux_task) ** 2, 2) / self.temperature2
            else:
                ...
                logits_reg = torch.bmm(aux_task, aux_center.permute([0,2,1])) / self.temperature2
