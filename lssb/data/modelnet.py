...
# GLOBALS
N_VIEWS = 25
VALID_MODALITIES = ['image', 'ptcld', 'image+ptcld', 'image+shape_embedding', 'image+shape_embedding+ptcld']

root_dir='./data/ModelNet40-LS'
...
class ModelNet(torch.utils.data.Dataset):
    ...
    def __init__(self, 
        ...
        self.LOWSHOT_JSON_PATH = '{}/modelnet_lowshot_train_dict.json'.format(root_dir)
        ...
        if use_aug and 'ptcld' in modality:
            ...
            if self.extra_args['use_random_SO3_rotation']:
                ...
                rot_trans = lssb_t.PointcloudRotate(axis=np.array([1, 0, 0]))
    ...
    def make_dataset(self):
        ...
        for cls in self.classes:
            ...
            for obj in class_objects:
                ...
                #get paths
                obj_img_path = os.path.join(self.BASE_PATH, 'renders',  cls, obj)
                obj_ptcld_path = os.path.join(self.BASE_PATH, 'pointclouds', cls, obj.replace('.obj','.npy'))

                #check paths
                exists = os.path.exists(obj_img_path) and os.path.exists(obj_ptcld_path)
    ...
    def __getitem__(self, idx):
        ...
        #get image and point cloud paths
        paths = self.get_paths(cls, obj, view)

        if 'ptcld' in self.modality:

            #load point cloud
            point_set = np.load(paths['ptcld']).astype(np.float32)
            ...
            # n_points x [X_pos, Y_pos, Z_pos, X_normal, Y_normal, Z_normal]
            # hard coded to only use point coordinates
        ...
        if 'embedding' in self.modality:
            ...
            # there are 10 features extracted per point cloud
            # each with a random sampling of 1024 points. Choose one of them
...
if __name__ == "__main__":
    ...
    dataset = ModelNet(split='train', modality='ptcld', extra_args=extra_args)
    dataset = ModelNet(split='val', modality='ptcld', extra_args=extra_args)
    dataset = ModelNet(split='test', modality='ptcld', extra_args=extra_args)
    
    dataset = ModelNet(split='train', modality='image')
    dataset = ModelNet(split='val', modality='image')
    dataset = ModelNet(split='test', modality='image')
